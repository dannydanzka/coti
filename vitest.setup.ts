...
// ── Prod-key tripwire — unit tests must NEVER touch production credentials ──
// If a live Stripe key or the prod Supabase project leaks into the test env,
// fail loudly instead of silently reaching real infrastructure. Then force
// deterministic fake credentials so boundary mocks never see a real secret.
const PROD_SUPABASE_REF = 'nhqnbowkeoeoiymqxzfm';
const hasLiveStripe = process.env['STRIPE_SECRET_KEY']?.startsWith('sk_live');
const hasProdSupabase = [
  process.env['NEXT_PUBLIC_SUPABASE_URL'],
  process.env['DATABASE_URL'],
  process.env['DIRECT_URL'],
].some((value) => value?.includes(PROD_SUPABASE_REF));
if (hasLiveStripe || hasProdSupabase) {
  throw new Error(
    `[vitest] Refusing to run: production credentials detected (${
      hasLiveStripe ? 'sk_live Stripe key' : 'prod Supabase project'
    }). Unit tests must never point at prod.`
  );
}
process.env['STRIPE_SECRET_KEY'] = 'sk_test_vitest_fake';
process.env['STRIPE_PUBLISHABLE_KEY'] = 'pk_test_vitest_fake';
process.env['STRIPE_WEBHOOK_SECRET'] = 'whsec_vitest_fake';
...
vi.mock('next/image', async () => {
  ...
  return {
    default: (props: { alt?: string; src?: unknown; [key: string]: unknown }) => {
      const { alt = '', blurDataURL, placeholder, priority, src, ...rest } = props;
      // StaticImageData imports arrive as objects; expose a string src for the DOM.
      const resolvedSrc =
        typeof src === 'string' ? src : ((src as { src?: string } | undefined)?.src ?? '');
      return React.createElement('img', { ...rest, alt, src: resolvedSrc });
    },
  };
});
...
vi.mock('next/dynamic', async () => {
  ...
  return {
    default: function mockDynamic() {
      // Render children only — spreading arbitrary component props (onSave, isOpen, …)
      // onto a DOM <div> produces React "unknown prop" warnings across the suite.
      const MockedComponent = ({ children }: Record<string, unknown>) => {
        return React.createElement(
          'div',
...
vi.mock('@prisma/client', () => ({
  /**
   * `Prisma.DbNull` is a runtime sentinel, not a type: clearing a nullable Json column
   * requires it, because plain `null` is how Prisma spells "leave it alone". Omitting it
   * here made any repository that clears a Json field throw on an undefined namespace.
   */
  Prisma: {
    DbNull: { _sentinel: 'DbNull' },
    JsonNull: { _sentinel: 'JsonNull' },
  },
