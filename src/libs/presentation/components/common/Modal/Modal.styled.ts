 ...
 *
 * Unified styles for modal variants:
 * - Standard modal with header, content, footer
 * - Confirm modal with icon, message, actions
 */
...
export const ModalOverlay = styled.div<StyledModalOverlayProps>`
  ...
  background-color: ${hexToRgba(color.neutral900, 0.6)};
  display: flex;
  inset: 0;
...
const sizeStyles: Record<'sm' | 'md' | 'lg' | 'xl' | 'full', ReturnType<typeof css>> = {
  full: css`
    max-height: 90vh;
    max-width: 90vw;
    width: 90vw;
...
export const BaseModalContainer = styled.div<StyledModalContainerProps>`
...
export const ModalConfirmTitle = styled(ModalTitle)`
...
export const ModalContent = styled.div<{ $noPadding?: boolean }>`
  flex: 1;
  overflow-y: auto;
  padding: ${({ $noPadding }) => ($noPadding ? '0' : spacing.md)};
`;
