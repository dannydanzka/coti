 ...
 *   - .constants.ts, .config.ts, .helpers.ts, .utils.ts
 *   - .test.ts, .mock.ts, /mocks/
 *   - .entity.ts, .styled.ts
 *
 * NOTE: .utils.ts/.utils.tsx are the helper layer themselves — pure
 *   transformation functions belong there, so they are exempt (suggesting
 *   "*.utils.helpers.ts" would be nonsensical double-suffixing).
 *
 * @version 1.1.0
 * @reviewed 2026-06-19
 */
...
export const componentOrganizationRule = {
  ...
  create(context) {
    ...
    /**
     * Boundary/contract layers whose defining responsibility IS translating
     * between adjacent shapes:
     *   - repository / mapper / slice → persistence/DTO ↔ domain entity mapping
     *   - use-case                    → request-param validation + response shaping
     *   - api route handler           → query parsing + HTTP response serialization
     * In this codebase those parse / format / transform / normalize functions are
     * co-located by established convention (~100 use-cases, ~136 routes) and are
     * intrinsic to the file's contract — NOT cross-cutting helpers.
     *
     * Also exempt are the app's dedicated utility layers, where a name-matched
     * function IS the file's purpose rather than an orchestration smell:
     *   - /services/      → heterogeneous infra utilities (matches the sibling
     *                       enforce-filename-convention rule's /services/ exclusion)
     *   - /libs/shared/   → logger, handleRequest, thunk, i18n, error helpers —
     *                       the foundation utility modules themselves
     *
     * The HELPERS POLICY (extract pure transforms to .helpers.ts) is therefore
     * skipped for these layers; it still fires for genuinely cross-cutting pure
     * logic in middleware, custom hooks, and React components/screens.
     * Interface/constant policies remain fully enforced everywhere.
     */
    const isBoundaryContractLayer =
      filename.endsWith('.repository.ts') ||
      filename.endsWith('.mapper.ts') ||
      filename.endsWith('.slice.ts') ||
      filename.endsWith('.use-case.ts') ||
      filename.endsWith('/route.ts') ||
      filename.endsWith('/route.tsx') ||
      filename.includes('/services/') ||
      filename.includes('/libs/shared/');
    ...
    return {
      ...
      VariableDeclaration(node) {
        ...
        node.declarations.forEach((declarator) => {
          ...
          if (!isBoundaryContractLayer && isFunction(declarator.init) && isHelperNaming(name)) {
            context.report({
              node: declarator,
