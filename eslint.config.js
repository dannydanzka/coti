...
import { routeDelegationRule } from './scripts/eslint-rules/route-delegation.js';
...
import { enforceFilenameConventionRule } from './scripts/eslint-rules/enforce-filename-convention.js';
...
export default [
  ...
  prettierConfig,
  {
    ...
    plugins: {
      ...
      custom: {
        rules: {
          ...
          'enforce-filename-convention': enforceFilenameConventionRule,
          ...
          'route-delegation': routeDelegationRule,
    ...
    rules: {
      ...
      'custom/enforce-filename-convention': 'warn',
      ...
      'custom/route-delegation': ['error', {
        /**
         * JUSTIFIED BOUNDARY ADAPTERS — infrastructure/IO routes, not business logic.
         * Tracked in .claude/plans/PLAN-ESLINT-ROUTE-DELEGATION.md.
         * Any NEW route importing prisma/repositories/supabase/stripe fails lint.
         * Do not add entries without a plan; every business route delegates to a use case.
         */
        allow: [
          // Storage maintenance: raw multi-bucket listing + multi-table URL scan (Supabase admin + prisma reads)
          'app/api/admin/cleanup/orphan-assets/route.ts',
          // Raw storage IO adapter (upload/delete to Supabase Storage)
          'app/api/upload/delete/route.ts',
          'app/api/upload/route.ts',
          // Stripe signature verification requires the Stripe SDK at the transport edge
  ...
  // global-error.tsx renders OUTSIDE the root layout — no ThemeProvider, no i18next, no Redux.
  // Inline styles and literal strings are a Next.js framework requirement here, not a code smell.
  {
  ...
  // ═══════════════════════════════════════════════════════════════════
  {
    ...
    rules: {
      ...
      // Playwright fixture pattern: `async ({}, use) => ...` is required to keep
      // the destructure type binding even when no fixture is consumed.
      'no-empty-pattern': 'off',
      // Cleanup teardown logs and factory error reporting are intentional in E2E.
      'no-console': 'off',
      // JSON.parse guard in api helpers is a legitimate silent-catch.
      'custom/no-try-catch-abuse': 'off',
      // Playwright `use(value)` callback is not a React hook.
  ...
  // Build/maintenance scripts and Playwright config.
  // These run in Node, not in the browser, and legitimately use console, await-in-loop, etc.
  {
    files: ['scripts/**/*.ts', 'playwright.config.ts', 'e2e/setup/global-teardown.ts'],
    rules: {
      'no-console': 'off',
      'no-await-in-loop': 'off',
      'no-restricted-syntax': 'off',
