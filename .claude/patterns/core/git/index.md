...
These policies are enforced mechanically by a global `pre-push` hook. The canonical
this tenant copy blocks direct pushes to `prod production master main` on **every repo on
the machine** — promotion happens only via PR. (`dev` is a working/integration branch
with direct push in this tenant repos; a stricter machine-local variant may also protect
`dev qa lab` for company repos.)

| Item | Value |
|------|-------|
| **Source of truth** | `core/hooks/git-pre-push.sh` (canonical hooks home — see `core/sops/local-hooks-strategy.md`) |
...
edit the `EXEMPT_REMOTES` regex inside the hook. Canonical default: `sovereignty`,
`soberania-del-codigo`.
