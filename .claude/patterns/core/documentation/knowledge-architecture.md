...
| Question | Source |
|----------|--------|
| How does this business work? (WHAT/WHY) | `<domain-repo>/products/{name}/domain/` |
...
> **Formal contract**: `layer-taxonomy.md` defines the 5 layers and uniform artifact types this map instantiates; `core/sops/knowledge-elevation.md` is the procedure for moving knowledge between them.
...
<domain-repo>/products/{name}/  ← Business knowledge (per product/client)
...
| **Domain pattern** | `<domain-repo>/products/{name}/domain/` | Permanent | A business rule changes |
...
## Domain Files (the domain repository)
...
```
<domain-repo>/products/{name}/domain/
...
> **Domain rules**: <domain-repo>/products/{name}/domain/{file}.md
...
- Business rule explanations (→ domain files in the domain repository)
...
| Business rule changed | ❌ Update the domain repository instead |
...
|---------------|---------------|
| **Business domain** (WHAT/WHY) | the product repo's own `domain/` |
| **Code patterns** (HOW — project-specific) | `.claude/patterns/business/` |
| **Technical patterns** (HOW — agnostic) | the sovereignty repo (this canon), delivered as `.claude/patterns/` |
...
| **C — Business rules superseded by the domain repo** | Code still valid, but rules now in the domain repository | Keep for code; never duplicate rules |
| **D — Migration candidate** | Agnostic content not yet in soberania | Migrate when opportunity arises |
| **E — Superseded** | Fully replaced by a domain-repository file | Mark as deprecated |
...
Create or update `<domain-repo>/context/plans/PLAN-KNOWLEDGE-ARCHITECTURE.md` defining the artifact map.

### Step 2 — Create domain/ in the domain repository
...
- Update Quick Reference table: separate domain (the domain repository) from code patterns (local)
- Update Business Model section to point to the domain repository's files
