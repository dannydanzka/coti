...
| [conduct/](conduct/) | **How an agent behaves** — the agnostic floor (speech, authorization, execution, judgment) + one nuance per domain (`development` · `management` · `content`). Injected at session start by the `sov-core` plugin; **not** a skill, because a floor cannot be probabilistic. Added 2026-08-22 |
