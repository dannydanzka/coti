...
Project-specific assertions (e.g., your-company's `assertion.textPresent`) live in this library, **not** scattered in each project's `__tests__/`. The library is the contract. The consuming repo documents its own `@test-helpers` API in its `.claude/rules/reference/`.
...
- the consuming repo's own `.claude/rules/reference/` — its `@test-helpers` API
