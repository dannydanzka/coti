...
## Shared DB project — schema-per-product

When several products share one Postgres/Supabase project, isolate each via **one Postgres
schema per product** (`?schema=<product>` in both URLs). (Elevated 2026-07-06 from presskit —
the Soverum shared "soverum" Supabase Pro project.)

**Critical gotcha**: `@prisma/adapter-pg` (the node-postgres driver adapter Prisma 7 uses)
**ignores `?schema=` in the connection string**. Queries silently target `public` unless the
schema is passed explicitly as a constructor option:
...
Apply this everywhere a `PrismaPg` instance is created (app client AND seed scripts).
