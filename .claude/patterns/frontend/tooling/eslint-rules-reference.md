...
> **v5.0 (2026-06-19)** — Catalog reconciled with rule files (was 19, actual 40). Imported from an upstream ruleset (FP-adjusted, adapted to our RTK/Vitest ecosystem): `no-utility-type-cast` (+Vitest mock exemption), `no-jsx-in-non-component-files` (+`.types.`), `enforce-filename-convention` (dir map adapted to this ecosystem, saga dirs removed). Modified: `no-try-catch-abuse` (+`console.warn` handler), `component-organization` (+HELPERS POLICY). See **Full Rule Inventory** below for the authoritative list.
...
> Rules **10, 11, 12** were imported from an upstream ruleset (2026-06-19) and adapted to our ecosystem. Rules **9, 23** were modified with the upstream's FP-adjustments. All five intro at `'warn'`. See dedicated sections at the end of this document.
...
| **Testing** | 10 | `essential-testing` + 9 test-file rules (rows 31–40 above) — full per-rule detail: [`eslint-testing-rules.md`](eslint-testing-rules.md) |
...
## Upstream-Imported Rules (v5.0, 2026-06-19)

Imported from an upstream ruleset and adapted to our RTK/Vitest/Next ecosystem. Each was validated against a real consumer repo (logic probes + real-exposure measurement) and introduced at `'warn'`.
...
## Modified Rules (upstream FP-adjustments, v5.0)

- **`no-try-catch-abuse`** — added `console.warn` to the valid-handler list (dev logging allowed by `no-console` config). Strictly more permissive; the upstream's redux-saga branch was **not** imported (irrelevant to RTK/Next).
