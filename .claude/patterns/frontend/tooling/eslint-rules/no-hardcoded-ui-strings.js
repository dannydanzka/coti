...
export const noHardcodedUiStringsRule = {
  ...
  create: (context) => {
    ...
    /**
     * Check if node is inside `export const metadata: Metadata = {...}` or a
     * `generateMetadata()` return object. Next.js resolves these server-side
     * for <head> tags before any client i18n context exists — there is no
     * useTranslation() available here, so this is a legitimate exception
     * (same rationale as the static legal-page skip above).
     */
    const METADATA_EXPORT_NAMES = new Set(['metadata', 'generateMetadata']);
    const isInMetadataExport = (node) => {
      let current = node.parent;
      while (current) {
        if (
          current.type === 'VariableDeclarator' &&
          current.id?.type === 'Identifier' &&
          METADATA_EXPORT_NAMES.has(current.id.name)
        ) {
          return true;
        }
        if (
    ...
    return {
      ...
      Literal: (node) => {
        ...
        // Skip Next.js metadata exports (server-only, no i18n context available)
        if (isInMetadataExport(node)) return;
